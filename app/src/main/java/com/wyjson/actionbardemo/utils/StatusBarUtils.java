...
public class StatusBarUtils {
     ...
     * @return
     */
    public static void setNavigationBarHeight(final Activity activity) {
        isNavigationBarExist(activity, new OnNavigationStateListener() {
            @Override
            public void onNavigationState(boolean isShowing, int bottom) {
                if (isShowing) {
                    setNavigationColor(activity);
                    View navigationBar = activity.findViewById(R.id.v_navigation_bar);
                    setNavigationBarHeight(activity, navigationBar);
                }
            }
        });
    }
     ...
     * @return
     */
    public static View setNavigationBarHeight(Activity activity, View navigationBar) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            if (navigationBar != null) {
                ViewGroup.LayoutParams lp = navigationBar.getLayoutParams();
                lp.height = getNavigationBarHeight(activity);
    ...
    public static void setColorForDrawerLayout(Activity activity, DrawerLayout drawerLayout, @ColorInt int color,
        ...
        if (!(contentLayout instanceof LinearLayout) && contentLayout.getChildAt(1) != null) {
            contentLayout.getChildAt(1)
                    .setPaddingRelative(contentLayout.getPaddingStart(), getStatusBarHeight(activity) + contentLayout.getPaddingTop(),
                            contentLayout.getPaddingEnd(), contentLayout.getPaddingBottom());
        }
        // 设置属性
